...
public interface CourseProviderRepository extends JpaRepository<CourseProvider, Long> {
  @Query("SELECT cp FROM CourseProvider cp WHERE cp.courseId = :courseId "
      + "AND cp.courseProviderId = :providerId")
  CourseProvider findByCourseIdAndProviderId(@Param("courseId") int courseId,
