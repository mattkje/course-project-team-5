...
public class UserController {
  ...
  public ResponseEntity<String> register(@RequestBody RegistrationRequest request) {
    String errorMessage = userService.tryCreateNewUser(
  ...
  public ResponseEntity<?> getProfile(
    ...
    if (userService.getSessionUser() != null) {
      if (userService.isAdmin()
          || userService.getSessionUser().user().getUsername().equals(username)) {
  ...
  public ResponseEntity<?> deleteUser(@PathVariable String username) {
    for (User user : userService.getAllUsers()) {
      if (user.getUsername().equals(username)) {
        UserWithCourses sessionUser = userService.getSessionUser();
        if (sessionUser != null
            && sessionUser.user().getUsername().equals(username) || userService.isAdmin()) {
  ...
  public ResponseEntity<?> refreshToken(@RequestBody TokenRefreshRequest tokenRefreshRequest) {
    try {
      ...
      UserDetails userDetails
          = userService.loadUserByUsername(jwtUtil.extractUsername(refreshToken));
    ...
    if (sessionUser != null) {
      if (userService.isPro()) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
            "This user already has the Pro role");
      }
