 ...
 * including their username, name, email, role, password,
 * account status, creation date, last update date and subscription expiry date.
 * Users can have different roles such as ADMIN, PRO, or USER,
 * each with specific permissions and privileges.
 * The enabled flag indicates whether the user's account is active.
 * The createdAt field stores the date when the user account was created,
 * and updatedAt stores the date
 ...
 * The users are able to hold a set of courses that they are enrolled in.
 * The courses are represented by the Course class.
 */
...
public class User {
  ...
  /**
   * Creates a new user with the specified username, email,
   * password, first name, last name, and phone number.
